...
This guide is for running Biometric Attendance Sync as a Windows service.
...
`requirements.txt` includes `pywin32` only on Windows through an environment marker. Use the virtual environment Python for source-based service commands so Windows registers the intended environment.

For packaged deployments, prefer the installer or the packaged `Biometric-Attendance-Sync-Service.exe`.
...
These tests mock Windows service APIs, ERPNext calls, and biometric devices. They do not install a Windows service and do not require a physical biometric device.

## Source-Based Service Commands
...
Do not install a duplicate packaged service if `ERPNextBiometricPushService` already exists.
...
1. Stop the service.
2. Pull or copy the updated code or installer package.
3. Activate the same virtual environment for source-based installs, or run the new installer for packaged installs.
4. Install changed dependencies if source-based deployment needs them.
5. Run unit tests.
...
The installer configures delayed automatic startup and recovery actions. Configure these manually only for source-based installations or when there is a separate deployment policy.
...
- write access to the configured `LOGS_DIRECTORY` or packaged ProgramData logs folder
